// src/services/AnalyticsService.js

// ✅ Your JSONBin ID:
export const BIN_ID = "68ffbc9243b1c97be9854ba3";

// Optional: if your bin requires a key, set it once per browser in DevTools:
// localStorage.setItem('prioritease.jsonbin.key', '<X-Master-Key>');
function getKey() {
  return localStorage.getItem('prioritease.jsonbin.key') || '';
}

export async function putSnapshot(payload, { retries = 2 } = {}) {
  if (!BIN_ID) throw new Error('BIN_ID is not set in AnalyticsService.js');
  const KEY = getKey();
  const url = `https://api.jsonbin.io/v3/bins/${BIN_ID}`;

  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      const res = await fetch(url, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          ...(KEY ? { 'X-Master-Key': KEY } : {}) // omit header if not needed
        },
        body: JSON.stringify(payload)
      });
      if (!res.ok) throw new Error(`PUT ${res.status}`);
      return await res.json();
    } catch (e) {
      lastErr = e;
      await new Promise(r => setTimeout(r, 400 * (attempt + 1)));
    }
  }
  throw lastErr || new Error('put failed');
}

export async function fetchLatestSnapshot() {
  if (!BIN_ID) throw new Error('BIN_ID is not set in AnalyticsService.js');
  const res = await fetch(`https://api.jsonbin.io/v3/bins/${BIN_ID}/latest`);
  if (!res.ok) throw new Error(`GET ${res.status}`);
  return res.json();
}

